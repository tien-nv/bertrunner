...
def eval(net,data_iter,criterion):
    ...
    for batch in data_iter:
        sents_embed,targets,_,doc_lens = process.process.make_features(process,batch,tokenizer=tokenizer,model_bert=model_bert)  
...
def train():
    ...
    for epoch in range(1,args.epochs):
        for i,batch in enumerate(train_iter):
            sent_embeds,targets,_,doc_lens = process.process.make_features(process,batch,tokenizer=tokenizer,model_bert=model_bert)
            targets = Variable(targets.float())
            if use_gpu:
